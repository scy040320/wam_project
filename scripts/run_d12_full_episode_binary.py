...
for episode_id in EPISODE_IDS:
    for condition in CONDITIONS:
        ...
        executed, queries, success, first_block, pending_refresh = 0, 0, False, True, None
        frames, decisions = [], []
        while executed < TASK_MAX_STEPS[SUITE] and not success:
            ...
            if condition == "object_shift" and first_block:
                env.sim.data.qpos[TOMATO_QPOS_ADDRESS:TOMATO_QPOS_ADDRESS + 3] += SHIFT_DELTA
        ...
        result_doc = {"episode_id":episode_id,"condition":condition,"success":success,"policy_steps":executed,"normal_query_count":queries,"extra_global_refresh_calls":sum(x["extra_wam_refresh_call"] for x in decisions),"decisions":decisions}
